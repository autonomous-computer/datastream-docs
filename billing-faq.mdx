...
Quota-counted routes return `429 Too Many Requests` with `code: "ai_query_quota_exceeded"` and structured `details` (`aiQueryQuotaLimit`, `aiQueryQuotaUsed`, `aiQueryQuotaRemaining`, `aiQueryQuotaResetAt`, `upgradeUrl`, `docsUrl`). Inspect `Omni-AI-Quota-*` headers on every quota-counted response for proactive monitoring.
