...
<CardGroup cols={2}>
  ...
  <Card title="Predictable subscription option" icon="credit-card" href="/plans-and-pricing">
    SEC API offers PAYG and optional subscriptions (Pro at $49/month, Team at $199/month) with included credits and discounted overages. AskEdgar's API is per-KB only.
  </Card>
...
AskEdgar's API is strictly per-KB. SEC API offers a renewable free monthly
allowance, pure PAYG, and optional subscriptions with included credits and
discounted overages. The conversion lever depends on payload size and call
volume:

| Profile                 | Volume                   | AskEdgar API (PAYG, per-KB) | SEC API option                 | SEC API $/mo |
| ----------------------- | ------------------------ | --------------------------- | --------------------------- | --------- |
| Light researcher        | 100 dilution-data calls  | ~$10.80 (≈$0.108/call)      | Free monthly allowance      | **$0**    |
| Hedge-fund analyst      | 600 dilution-data calls  | ~$64.80                     | PAYG (600 × $0.01)          | **$6.00** |
| Hedge-fund analyst      | 600 dilution-data calls  | ~$64.80                     | Pro                         | **$49**   |
| Small-cap research team | 5,000 dilution-data calls| ~$540                       | Team                        | **$199**  |
...
published pricing. The free allowance renews monthly with hard caps; ongoing
PAYG usage requires card-backed activation, and Pro/Team add monthly included
usage credits plus discounted overage rates.
