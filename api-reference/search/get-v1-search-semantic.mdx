...
- `requestId`
- `traceparent`
- `Omni-Meter-Class: semantic_search`
...
<Prompt>
Use Omni Datastream GET /v1/search/semantic to run semantic or hybrid search across filing section content. Pass `q`, optional filters like `ticker`, `cik`, `form`, `filing_year`, `fy`, `year`, and choose `mode` (`keyword`, `semantic`, or `hybrid`) based on the retrieval task. Preserve `requestId`, `traceparent`, and `Omni-Meter-Class` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
...
- preserve `Omni-Meter-Class` when present so downstream billing and usage reasoning stays intact
