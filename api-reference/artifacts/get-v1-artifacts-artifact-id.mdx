...
<Prompt>
Use SEC API GET /v1/artifacts/{artifact_id} to get artifact. Preserve `id`, `kind`, `status`, `manifest`, `provenance` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
