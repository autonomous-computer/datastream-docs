...
<Prompt>
Use SEC API POST /v1/artifacts to create artifact bundle. Preserve `id`, `kind`, `status`, `downloadUrl`, `storageMode`, `manifest`, `provenance` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
