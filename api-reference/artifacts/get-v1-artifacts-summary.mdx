...
description: "Artifact storage summary"
---
...
<Prompt>
Use SEC API GET /v1/artifacts/summary to artifact storage summary. Preserve `orgId`, `rows[].kind`, `rows[].status`, `rows[].storageMode`, `rows[].count`, `rows[].totalBytes`, `requestId`, `traceparent` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
