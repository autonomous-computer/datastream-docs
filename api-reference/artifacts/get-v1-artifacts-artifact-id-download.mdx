...
Returns the saved payload directly when it is stored inline. When the payload is held in object storage, the response is a `302` redirect to a short-lived signed URL, so your client must follow redirects. Retain the artifact ID and manifest checksum alongside downloaded content.
...
<Prompt>
Use SEC API GET /v1/artifacts/{artifact_id}/download to download artifact. Preserve `artifact ID`, `manifest checksum`, `200 payload or 302 redirect` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
