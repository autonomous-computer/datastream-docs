...
description: "Shareholder voting results"
---
...
<Prompt>
Use SEC API GET /v1/events/voting-results to shareholder voting results. Preserve `requestId`, `traceparent`, `per-row provenance`, `per-row freshness`, `per-row materialization` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
