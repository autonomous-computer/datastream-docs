...
<Prompt>
Use SEC API GET /v1/entities to search entities. Preserve `data`, `hasMore`, `nextCursor`, `requestId`, `traceparent` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
