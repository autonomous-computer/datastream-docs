...
<Prompt>
Explain that monitor name and query are required. Describe filters, search mode, startAt, and delivery as optional fields; if a destination is supplied, use either webhookUrl or delivery, not both. Direct the user to create the monitor in the signed-in SEC API dashboard, and do not claim a schedule or delivery outcome that the response does not prove.
</Prompt>
