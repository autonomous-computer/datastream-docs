...
description: "Create agent bootstrap token"
---
...
<Prompt>
Use SEC API POST /v1/agent/bootstrap_tokens to create agent bootstrap token. Preserve `requestId`, `id`, `tokenPrefix`, `scopes`, `expiresAt`, `usedAt` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
