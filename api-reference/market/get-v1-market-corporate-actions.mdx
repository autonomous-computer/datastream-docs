...
<Prompt>
Use Omni Datastream GET /v1/market/corporate-actions to return rights-reviewed corporate actions used to normalize price history and event analysis. Preserve `requestId`, `traceparent`, `provenance`, `freshness`, `materialization` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
