...
<Prompt>
Use SEC API GET /v1/market/reference to security reference data. Preserve `symbol`, `cik`, `compositeFigi`, `shareClassFigi`, `exchange`, `securityType`, `asOf`, `freshness`, `provenance`, `sourceRights`, `revision`, `requestId`, `traceparent` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
