...
<Prompt>
Use SEC API POST /v1/analytics/query to run analytics query. Preserve `requestId`, `traceparent`, `dataset`, `dimensions`, `filters`, `timeWindow`, `sort`, `rowCount`, `warnings`, `provenance` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
