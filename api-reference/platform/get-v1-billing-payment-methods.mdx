---
title: "GET /v1/billing/payment-methods"
description: "List payment methods"
---

List payment methods

<Info>
Audience: application and coding agent.
</Info>

## Coverage and source scope

List the current organization's saved payment methods, primary first.

## Canonical metadata

- `requestId`
- `traceparent`

## Example request

<RequestExample>
```bash
curl -X GET \
  -H "Authorization: Bearer $SECAPI_BEARER_TOKEN" \
  -H "secapi-version: 2026-03-19" \
  "https://api.secapi.ai/v1/billing/payment-methods"
```
</RequestExample>

<Info>
Payment-method management requires a WorkOS bearer session with `billing:manage`. API keys are rejected so saved payment methods stay limited to interactive billing administrators.
</Info>

## Example response

<ResponseExample>
```json
{
  "requestId": "req_2ZK8Q1W9F4M6P7R3"
}
```
</ResponseExample>

## Give this prompt to your agent

<Prompt>
Use SEC API GET /v1/billing/payment-methods to list payment methods. Preserve `requestId`, `traceparent` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>

## Failure posture

- treat non-2xx responses as contract-aware failures, not free-form errors
- preserve `requestId` and `traceparent` in logs and downstream reports
- if provenance or freshness metadata is present, return it unchanged so trust is not lost in the handoff
