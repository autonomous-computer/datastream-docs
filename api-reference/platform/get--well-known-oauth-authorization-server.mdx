...
<Prompt>
Use SEC API GET /.well-known/oauth-authorization-server to oAuth authorization server metadata. Preserve `requestId`, `traceparent` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
