...
description: "Portfolio risk model"
---
...
```json
{
  ...
  "securities": [
    ...
    },
    {
      ...
      "ctevShare": 0.04,
      "ctev": 0.0016,
      "note": "Outside the ~4k US factor model universe — beta-only / specific-risk-floor treatment."
    }
  ],
  "coverage": {
  ...
  "disclosures": [
    "Ex-ante risk uses an EWMA factor covariance (~2y half-life, weekly) over the style/sector/industry factor universe (2015→).",
    "Model coverage: 94% of portfolio NAV is inside the ~4k US factor model universe. Unmodelled names use a beta-only / specific-risk-floor treatment.",
...
<Prompt>
Use SEC API POST /v1/portfolio/risk to portfolio risk model. Preserve `requestId`, `traceparent` in the output. Return a concise markdown summary plus the exact structured payload fields that a downstream engineer or agent should keep using this result.
</Prompt>
