...
- Resolve an issuer with GET /v1/entities/resolve when the workflow starts from a name, ticker, CIK, or identifier. Carry the returned identity forward; do not guess a CIK. Do not treat GET /v1/entities/edgar as an identity peer — that route is EDGAR catalog search.
- After resolve, pick a live store (filings, factors, or macro) or POST /v1/intelligence/query for a named intel job. Letters are gated — do not make them a first curl. Check availability before curling gated surfaces; they return 404 when dark.
...
- `/v1/statements/segmented-revenues` — Segment revenue history
- `/v1/statements/share-float` — Public float
- `/v1/statements/all` — All financial statements
- `/v1/entities/resolve` — Resolve entity identifier
- `/v1/entities` — Search entities
- `/v1/filings` — List filings
- `/v1/filings/latest` — Latest filing
- `/v1/filings/latest/render` — Render latest filing
- `/v1/filings/latest/sections/{section_key}` — Latest filing section
- `/v1/filings/latest/risk-categories` — Risk factor categories
- `/v1/filings/{accession_number}` — Get filing by accession
- `/v1/filings/{accession_number}/sections/{section_key}` — Filing section by accession
