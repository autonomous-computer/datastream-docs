...
| `402`, `billing_required`, or a budget code | Inspect billing and limits with the same credential, then resolve the stated account condition. Read `details.reason` to identify which condition it is. |
| `402 billing_required` with `details.reason` of `starter_grant_exhausted` | The non-renewing starter grant is spent and the account has no payment method. Follow `details.addPaymentMethodUrl` or `details.topUpUrl`; `details.nextSteps` lists the self-serve options. Watch `SECAPI-Grant-Status` on earlier responses to catch this before it blocks requests. |
...
`billing_budget_approval_required` means a configured approval threshold stopped the request. `billing_budget_hard_cap_exceeded` means a configured hard cap stopped it. `ai_query_quota_exceeded` means the applicable AI-query allowance is exhausted. These are control signals, not transient failures: avoid a retry loop until the returned condition changes. See [Agent billing](/agent-billing), [Plans and pricing](/plans-and-pricing), [GET /v1/limits](/api-reference/platform/get-v1-limits), and [API keys, usage, and billing](/auth-and-pricing).
