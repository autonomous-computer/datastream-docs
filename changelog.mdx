...
<Update label="April 28, 2026" description="Monitor → email auto-fire (Dagster cron + retry queue + daily summary)" tags={["OMNI", "Release"]}>
Email destinations on monitors now fire automatically. The OMNI Dagster cron sweeps active monitors every 15 minutes and dispatches new matches via the email channel shipped in the prior release. Includes a retry state machine, per-monitor + per-org rate limits, and a 23:55 UTC daily summary for over-the-cap matches.

### Highlights

- 15-minute Dagster cron sweep dispatches new monitor matches via Resend; idempotent against (monitor_id, idempotency_key) so re-runs on overlapping cursors don't duplicate emails
- Retry state machine with 30s → 5m → 30m backoff; 3 attempts max before permanent_failed
- Per-monitor hourly cap (OMNI_MONITOR_DISPATCH_RATE_LIMIT_PER_HOUR=100) + per-org daily cap (OMNI_MONITOR_DISPATCH_DAILY_CAP=1000); over-cap rows persist as rate_limited and feed the daily summary
- Daily summary email at 23:55 UTC rolls up rate-limited matches per (org, recipient); idempotent per UTC date
- monitor.match Datastream events fire on every dispatch — org-level webhook subscribers receive the signal via webhook_endpoints regardless of email outcome
- Transient 'sending' status pattern keeps Postgres locks short during Resend HTTP calls — aligned with OMNI-3414 connection-pool work
- Stage 1 ships dormant behind OMNI_MONITOR_DISPATCH_ENABLED=false; Stage 2 = post-merge env flip
- Legacy monitor.webhookUrl column is now formally deprecated — see OMNI-3436 for removal timeline
</Update>

<Update label="April 27, 2026" description="feat(bench): multi-ticker pool + variance_flag for OMNI-3099 (numbers refresh deferred) + 2 more" tags={["OMNI", "Release"]}>
