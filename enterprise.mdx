...
Sandbox and Pay as you go include unlimited users. Existing Team subscriptions, where still active, share:
...
Key limits follow the account catalog. Custom and grandfathered Team accounts may include more keys than PAYG:
...
SEC API provides signed, logged, at-least-once webhook delivery with manual replay for retained delivery attempts. Automatic retry and endpoint auto-pause are not generally available yet; [Webhook Delivery](/webhook-sla) is the source of truth for the current contract.
