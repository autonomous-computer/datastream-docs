...
- `sandbox_grant`: renewable monthly free allowance for low-volume testing
- `payg`: self-serve metered usage with prepaid credits
- `personal`: Pro self-serve subscription for one seat, included credits, and discounted API/CLI/MCP/add-in usage
- `team`: self-serve subscription for shared organization usage, pooled included credits, five included seats, and ten included API keys
...
For internal research, private tools, notebooks, and agent workflows that do not redistribute output, start with Pay As You Go, Pro, or Team and use `GET /v1/billing` plus `POST /v1/billing/quote` to size usage before scaling.
